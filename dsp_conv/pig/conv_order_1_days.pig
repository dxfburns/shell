...
IN_DATA = load '$indir' using PigStorage('\t', '-noschema') as (
	...
	sid:chararray,       
	adid:chararray,       
	order_number:chararray,       
	product_number:chararray,       
...
IN_DATA_FILTER = foreach IN_DATA_REGFILTER generate conv_date,ip,url,uid,adid,order_number,product_number,product_name,price,flatten(urlmatchreg(adid, url, '$eachreg')) as conv_url:chararray; 

COVN_TMP = FILTER COVN_TMP_DATA by calcindex == 'CV';
...
RES_FIN = foreach EX_IN_RES generate
        ...
        EX_DATA::ua as ua,
		IN_CONV_JOIN::IN_DATA_FILTER::order_number as order_number,
		IN_CONV_JOIN::IN_DATA_FILTER::product_number as product_number,
		IN_CONV_JOIN::IN_DATA_FILTER::product_name as product_name,
		IN_CONV_JOIN::IN_DATA_FILTER::price as price,

...
COVN_TMP = FILTER COVN_TMP_DATA by calcindex == 'UV';
...
GROUP_RES = group RES_FIN by (visit_date,url,iid,uid);

DISTINCT_RES = foreach GROUP_RES {
	ordertempresult = order RES_FIN by conv_date;
	limittempresult = limit ordertempresult 1;
	generate flatten(limittempresult);
}
...
GEO_JOIN = join DISTINCT_RES by (limittempresult::countryid,limittempresult::provinceid,limittempresult::cityid) LEFT OUTER, DATA_GEO by (country_id,province_id,city_id);

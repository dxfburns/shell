...
define urlmatchregfilter com.adsage.pig.bank.udf.UrlMatchesClickFilter(); 
define urlmatchregfunc com.adsage.pig.bank.udf.UrlMatchesClickFunc(); 
...
COVN_TMP = load '$conv_tmp_dir' using PigStorage('\t', '-noschema') as (
...
IN_DATA_REGFILTER = FILTER IN_DATA_URLFILTER by urlmatchregfilter(url, '$eachreg', eventid, adid);
IN_DATA_FILTER = foreach IN_DATA_REGFILTER generate conv_date,ip,url,uid,adid,flatten(urlmatchregfunc(url, '$eachreg', eventid, adid)) as conv_url:chararray;

--COVN_TMP = FILTER COVN_TMP_DATA by calcindex matches '[CL]:*';
